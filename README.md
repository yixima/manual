...
```
dist/                    ← 配布物（これを各所に貼る）
  L0_core_card_v22.md      コアカード（152行）★常時コンテキストに載せる中核
  L0_core_card_mini_v22.md 短縮版（79行）設定欄の文字数上限で全文が入らない場合の代替
  L1_manual_v22.md         本編（全条文・無省略・124条項）
  L2_records_v22.md        失敗記録（27件）・附則・付録
  manual_v22_all_in_one.md すでに開いているセッションに添付する1ファイル（L0+L1+L2）
  handover_template_v22.md 引き継ぎのテンプレート（記録が残らない [Chat]/[Cowork] 用）
...
8. **引き継ぎが「記憶から書き写す作業」だった。**（v22 で作り替えた）
...
./tools/test_tools.sh                                                                      # スクリプトの起動検証（66件）
```
...
1. **claude.ai → 設定 →「Claudeへの指示」** に `dist/L0_core_card_v22.md` の全文を貼る → **すべての会話・すべてのプロジェクト**に効く
2. **Cowork → 設定 → Cowork →「グローバル指示」** に同じ全文を貼る → **すべての Cowork セッション**に効く

文字数の上限で入らない場合は `dist/L0_core_card_mini_v22.md`（4,136字）を使う。
...
その環境では `dist/handover_template_v22.md` を使い、**節目ごとに同じファイルを作り直す**（§0-5）。
...
| 機械的強制 | 無し | 無し | **6種（`[Code]`）** |
| 自動テスト | 無し | 無し | **111件（フック45・スクリプト66）** |
