...
```
dist/                    ← 配布物（これを各所に貼る）
  L0_core_card_v33.md      コアカード（152行）★常時コンテキストに載せる中核
  L0_core_card_mini_v33.md 短縮版（79行）設定欄の文字数上限で全文が入らない場合の代替
  L1_manual_v33.md         本編（全条文・無省略・124条項）
  L2_records_v33.md        失敗記録（29件）・附則・付録
  manual_v33_all_in_one.md すでに開いているセッションに添付する1ファイル（L0+L1+L2）
  handover_template_v33.md 引き継ぎのテンプレート（記録が残らない [Chat]/[Cowork] 用）
...
8. **引き継ぎが「記憶から書き写す作業」だった。**（v22 で作り替えた。以後 v33 まで改良を重ねている）
...
```bash
python3 tools/audit_activation.py dist/L1_manual_v33.md --records dist/L2_records_v33.md   # 発動構造の検査
...
1. **claude.ai → 設定 →「Claudeへの指示」** に `dist/L0_core_card_v33.md` の全文を貼る → **すべての会話・すべてのプロジェクト**に効く
2. **Cowork → 設定 → Cowork →「グローバル指示」** に同じ全文を貼る → **すべての Cowork セッション**に効く

文字数の上限で入らない場合は `dist/L0_core_card_mini_v33.md`（4,136字）を使う。
...
その環境では `dist/handover_template_v33.md` を使い、**節目ごとに同じファイルを作り直す**（§0-5）。
