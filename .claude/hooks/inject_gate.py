...
**1分を超える見込みの処理は、始める前に「何を・約何分」を1行出す**（見込みを出さない長い処理は、ユーザーには停止として見える）。
...
DEFAULTS = {
    # 負荷スコアのしきい値（主指標）
    "notice_score": 15.0,     # 注意水準：頭の片隅に置くだけ。申告も中断も不要（v61 で 8→15）
    "report_score": 35.0,     # 申告水準（v61 で 20→35。スコア44.9で支障なしの実測＋ユーザー指示。ただし劣化実測のスライド事例≒36は捕まえる）
