...
# R1b【型H】本文で先の作業を宣言しながら、状態行は「完了」と言っている（L1 §2-17／§2-15）
# ユーザーの指摘（2026-09-03）：「作業が残っているのに、理由も書かずにターンを終える。
# こういう時も多い」。R1 は**末尾**の着手宣言しか見ないため、
# 本文の途中で「次に〇〇します」と書き、末尾は「状態：完了」で締める形をすり抜けていた。
# **宣言した作業があるのに「完了」と言うのは、状態の誤報である。**
# 誤検知を避けるため、**引用・鉤括弧・コードを除いた本文**だけを見る（§原文は対象外）。
RE_DECL_ANY = re.compile(
    r'(これから|続けて|次に|引き続き|この後|あとで|後ほど)[^。\n]{0,40}?'
    r'(します|着手します|実行します|進めます|作成します|開始します|反映します|発行します)')
