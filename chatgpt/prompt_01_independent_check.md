...
**使い方**：このファイルの「▼ここから」以降を ChatGPT に貼り、続けて **本編と記録の全文**（配布URL＝ https://raw.githubusercontent.com/yixima/manual/main/latest/manual_all_in_one.md の「本編（L1）」以降。版番号は書かない——**貼った時点の最新版が対象である**）を貼る。
...
4. **失敗記録の捕捉率**：記録（L2）の各失敗記録について、その「再発防止＝§X」に挙がる条項が**すべて** 2 の集合に含まれるかを判定し、含まれる記録の件数 ÷ 記録の総件数を答えてください。
