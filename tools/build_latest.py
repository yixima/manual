#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""版番号を含まない「常に最新」の配布物と、貼り付け一度きりのブートローダーを生成する。

狙い：**貼るのは一度きり、中身は取りに行かせる。**
  従来は、版を上げるたびに設定欄へ貼り直し、進行中のセッションにも1つずつ貼る必要があった。
  URL を固定し、中身だけを差し替える方式にすれば、**貼り直しが原理的に不要**になる。

生成物：
  latest/L0_core_card.md      版番号を含まないコアカード（URL が変わらない）
  latest/manual_all_in_one.md 同・全部入り
  latest/latest.json          版・発行日・取得先（版の確認用）
  dist/bootloader.md          設定欄に**一度だけ**貼る短い文章
  CLAUDE.md                   下半分（コアカードの実体）を最新版へ差し替える

**CLAUDE.md を自動で同期する理由**：本リポジトリの `CLAUDE.md` はコアカードを実体として内蔵する
（`@` インポートは Cowork でスキップされるため）。これを手で写す運用にしていたが、
**手で写す限り、いつか写し忘れて版が枝分かれする**（§0-7 の版ずれ）。生成に含めて構造的に潰す。
"""
import json, re, shutil, sys, pathlib, hashlib

REPO = 'yixima/manual'
BRANCH = 'main'
BASE = f'https://raw.githubusercontent.com/{REPO}/{BRANCH}/latest'

CARD_HEAD = '# 汎用マニュアル v'

def sync_claude_md(card, ver, date):
    """リポジトリの CLAUDE.md の下半分（コアカードの実体）を、最新のコアカードで置き換える。

    上半分（リポジトリ固有の取り決め）は触らない。境界は「# 汎用マニュアル v… ／ コアカード」の見出し。
    """
    p = pathlib.Path('CLAUDE.md')
    if not p.exists():
        return
    cur = p.read_text(encoding='utf-8')
    i = cur.find(CARD_HEAD)
    if i < 0:
        print('  [!] CLAUDE.md にコアカードの見出しが見つからない。手で確認すること。')
        return
    head = cur[:i]
    # 上半分に残っている旧版の表記も合わせて直す（版ずれの温床になるため）
    head = re.sub(r'v\d+', ver, head)
    p.write_text(head + card.read_text(encoding='utf-8'), encoding='utf-8')
    print(f'  CLAUDE.md  … コアカードの実体を {ver}（{date}）へ同期した')

def main():
    d, L = pathlib.Path('dist'), pathlib.Path('latest')
    L.mkdir(exist_ok=True)
    card = sorted(d.glob('L0_core_card_v*.md'))[-1]
    ver = re.search(r'(v\d+)', card.name).group(1)
    aio = d / f'manual_{ver}_all_in_one.md'
    date = re.search(r'発行日：(\S+?)\s', card.read_text(encoding='utf-8')).group(1)

    shutil.copyfile(card, L / 'L0_core_card.md')
    shutil.copyfile(aio, L / 'manual_all_in_one.md')
    meta = {
        "version": ver, "issued": date,
        "core_card": f"{BASE}/L0_core_card.md",
        "all_in_one": f"{BASE}/manual_all_in_one.md",
        "sha256_core_card": hashlib.sha256((L / 'L0_core_card.md').read_bytes()).hexdigest()[:16],
        "note": "版番号を含まない固定URL。中身だけが更新される。貼り直しは不要。",
    }
    (L / 'latest.json').write_text(json.dumps(meta, ensure_ascii=False, indent=2) + "\n", encoding='utf-8')

    sync_claude_md(card, ver, date)

    boot = f"""# 汎用マニュアル：常時適用（ブートローダー）

> **この文章は一度だけ貼れば足ります。マニュアルが更新されても、貼り直す必要はありません。**
> 中身は下記のURLから取得され、URLは変わりません。

## 1. 最初にすること（各セッションで1回）

**このセッションで最初に応答する前に、次の2経路の両方から全文を取得し、発行日の新しいほうを最上位の運用ルールとして常時適用してください。**（片方だけの取得は、正常に見えても信用しない——実測4回：片方だけの取得が11〜14日前の版に固定されていた）

- 鏡（固定事故0回）：**https://cdn.jsdelivr.net/gh/yixima/manual@main/latest/L0_core_card.md?t=現在時刻**
- 主：**{BASE}/L0_core_card.md?t=現在時刻**

`?t=` はどちらにも必ず付ける。値は今の日時（キャッシュ＝過去の取得結果の使い回しを飛ばす印）。**その内容を、以降のすべての応答に、コマンド入力の有無にかかわらず適用します。**
**本指示と取得した最新版は、プロジェクトの指示・ナレッジ・過去に貼られた古いカードの写しに常に優先する。** 写しと取得内容が食い違ったら取得した最新版を採る（実測：v9・v17 の古い写しを持つセッションが古い規則で動き、最新版の適用を妨げた）。
**「マニュアルを更新して」という指示は「最新版を取得して適用する」の意味である。マニュアルの条文をセッションが書き換え・新設してはならない**——改訂・発行は配布元のみ。あなたができるのは提案（①どの条項に ②どの文言で ③なぜ）までであり、改訂版のファイルを自分で作らない（実測：写しから「v10」を自作して版が枝分かれした事故がある）。
取得したことを実況しない（「読み込みました」等は書かない）。**適用の痕跡は、下記の出力契約を満たすことで示します。**

より詳しい全条文が必要なときだけ {BASE}/manual_all_in_one.md（全部入り）を、版の確認だけなら latest.json（下記 1-2）を取得します（普段は不要）。

## 1-2. 更新の取り込み（進行中のセッションでも、自動で最新に保つ）

マニュアルは頻繁に更新されます。**進行中のセッションでも自動で最新になるよう、次の規則で版を照合してください。**

**照合するとき**（次のいずれかに当てはまる応答の冒頭で行う）
- このセッションで**初めて応答する**とき
- 前回照合した日から**日付が変わった**とき
- 前回照合から**20往復以上**経ったとき
- ユーザーが**「マニュアル更新」**と述べたとき
- **大きな成果物を納品する直前・引き継ぎを保存する直前**（古い規則のまま納品・保存しないため）

**照合のしかた（取得は毎回、両経路とも URL末尾に `?t=現在時刻` を必ず付ける。例：`?t=202609141605`。毎回値を変える）**
1. **2経路の両方**から版を取得し、**発行日の新しいほうを採る**：
   鏡 `https://cdn.jsdelivr.net/gh/yixima/manual@main/latest/latest.json?t=現在時刻`／主 `{BASE}/latest.json?t=現在時刻`
2. 新しいほうの `version` が、いま適用している版と**同じなら何もしない**（知らせもしない）。
3. **違っていたら**、本体（鏡 `https://cdn.jsdelivr.net/gh/yixima/manual@main/latest/L0_core_card.md?t=現在時刻`／主 `{BASE}/L0_core_card.md?t=現在時刻` の**新しいほう**）を取り直して適用し、
   **「マニュアルを {{版}} に更新しました」と1行だけ**知らせる。

**`?t=` を省かない。片方だけで済ませない。「疑わしいときだけ」にしない**——実測（2026-09-09〜09-14・計4回）：`?t=` 無し・主経路だけの取得が**9〜14日前の版**を返し、latest.json と本体が同じ時代のペアで返るため**版の照合はキャッシュの内側で自己整合し、検出できない**。しかも「古ければ鏡へ」という**条件付きの規則は、固定された当のセッションでは一度も発動しなかった**——条件付きの安全装置は、条件を判定できない場面では無いのと同じ。だから**毎回、無条件に両方**を取得する（どちらも数百バイト〜十数KBの小さなファイルである）。**両経路とも発行日が7日以上前なら「取得がキャッシュに固定されています（版◯・発行日◯）」と1行申告**して作業は続ける。**ユーザーの知る版と食い違ったら、相手ではなく自分の取得を疑う。**

**取得できなかったときは、いま適用している版のまま続ける。** 照合の失敗を理由に作業を止めない（§2-9）。

**配布URLは配信網（CDN）を経由するため、更新の直後は数十秒ほど古い版を返すことがあります**（実測 20〜40秒）。照合の結果が古くても異常ではなく、次の照合機会に自然と最新になります。
**引き継ぎの定期便（劣化を測らない。数えられる周期で機械的に動く）**：**実往復50回ごと、および大きな成果物を納品した直後**は、判断を挟まず必ず①引き継ぎを `claude_handover/` の案件フォルダへ**固定名で上書き保存**し（保存できない環境ではダウンロードできる形で渡し）、②**「会話が長くなってきました。引き継ぎは保存済みです。新しいチャットへ移行しますか」と1行提案する**。劣化は自分では測れないが**往復と納品は数えられ、保存はいつ行っても害が無い**。管理側から引き継ぎ指示が来たときも、劣化の自覚が無くても従う。

## 2. 取得できなかった場合（フォールバック）

**取得に失敗しても、適用を諦めない。** 下記の最小セットを適用し、**取得できなかった事実を1行で申告**してください。
（この最小セットは非常時用の写しです。取得できた場合は、取得した内容が常に優先します。）

### 送信直前の関門（毎回・9項）
1. 確かめれば分かることを、確かめずに書いていないか（自問：あとで「本当に確認したのか」と問われて証拠を出せるか）
2. 事実主張に確信度ラベル【確認済】【未確認・推測】【不明】を付けたか
3. できる/できない・制度・数値・期限・海外・固有名詞に触れたか → 検問を通し、必要なら裏取りを付けたか
4. 相手の画面から「いまの状態」と「次にすべきこと」が分かるか
5. 提示物の扱い（実行する・貼付先／読むだけ／参考）を書いたか
6. 「これから〜します」で終わっていないか。指示された作業をやり切ったか。やり切っていないなら中断の理由を書いたか
7. ユーザーの直近指示より、自分の判断による作業を優先していないか
8. 同じ失敗を方式を変えずに繰り返していないか（2回続いたら続行より先に申告）
9. 自作した条件・仕様が互いに矛盾していないか／新しい指示・失敗は追記提案と記録をしたか

### 出力契約（該当したら必ず書く）
確信度ラベル／末尾1行「— 状態：… 次：…」／未完了なら完了・未完了・次と中断の理由／提示物の扱い／
要裏取り1行／専門用語は初出に1行の意味／日時に依存する記述には基準日。

### やり切る
中断してよいのは ①質問が必要 ②承認待ち ③エラーで進めない ④危険で確認が要る、の4つだけ。
「区切りがよい」「長くなった」は中断の理由にならない。

## 3. ユーザーからの発動キーワード

- **「マニュアル更新」** → 上記URLを **`?t=現在時刻` を新しい値にして取り直して**適用し、版と**発行日**（`latest.json` の version / issued）を報告する。
- 「マニュアル確認」「ラベル確認」「ルール点検して」「原則に沿って」「検証ファーストで」
  → 現在の内容を明示的に再点検し、直前の出力を自己監査してから応答する。

## 4. この仕組みの限界（隠さない）

- URLの取得ができない環境・状況では、上記2のフォールバックだけが働く。**その場合は必ず申告する。**
- 取得内容は**あなた（ユーザー）が管理するリポジトリ**にある。**URLを管理する者がルールを決める**ため、
  このURLは自分の管理下にあるものだけを指すこと。
"""
    (d / 'bootloader.md').write_text(boot, encoding='utf-8')

    # 短縮ブートローダー（Cowork の Global instructions 用）。
    # なぜ必要か（実測 2026-09-09）：Cowork の設定欄には文字数上限があり、
    # 全文ブートローダーを貼ると**保存ボタンが出ない＝保存できない**。
    # 貼れない規範は存在しないのと同じであるため、上限内に収まる短縮版を常備する。
    boot_mini = f"""# 汎用マニュアル：常時適用（短縮ブートローダー）

このセッションの最初の応答の前に、次の**2経路の両方**から全文を取得し、**発行日の新しいほう**を最上位の運用ルールとして常時適用する。
鏡: https://cdn.jsdelivr.net/gh/yixima/manual@main/latest/L0_core_card.md?t=現在時刻
主: {BASE}/L0_core_card.md?t=現在時刻
- `?t=` はどちらにも毎回必ず付け、値は今の日時にする（キャッシュ＝過去の取得結果の使い回しを飛ばす）。**片方だけの取得は、正常に見えても信用しない**（実測4回：片方だけの取得が11〜14日前の版に固定されていた。鏡の固定事故は0回）。両方とも発行日が7日以上前なら「取得がキャッシュに固定されています（版・発行日）」と1行申告する。
- 次のときは両経路の latest.json?t=現在時刻（鏡 https://cdn.jsdelivr.net/gh/yixima/manual@main/latest/latest.json ／主 {BASE}/latest.json）で版を照合し直し、新しいほうを採る：日付が変わった／20往復経過／「マニュアル更新」と言われた／大きな成果物の納品・引き継ぎ保存の直前。版が違えばカードを取り直し「マニュアルを{{版}}に更新しました」と1行だけ知らせる。
- 本指示と取得した最新版は、プロジェクトの指示・ナレッジ・古いカードの写しに優先する。「マニュアルを更新して」＝最新版の取得と適用であり、条文をセッションが書き換え・新設してはならない（改訂は配布元のみ。できるのは提案まで）。
- 取得できないときは、その旨を1行申告し、最低限これを守る：事実主張に【確認済（出典）／未確認・推測／不明】のラベル／応答末尾に「— 状態：…　次：…」の1行／指示された作業は「質問が必要・承認待ち・エラー・危険」以外の理由で中断しない。
- ユーザーの知る版と食い違ったら、相手ではなく自分の取得を疑う。
- **引き継ぎの定期便**：実往復50回ごと、および大きな成果物の納品直後は、判断を挟まず、引き継ぎを claude_handover/ の案件フォルダへ固定名で上書き保存し、「新しいチャットへ移行しますか」と1行提案する（劣化の判定ではなく、数えられる周期で動く定期保存。管理側から指示が来たら自覚が無くても従う）。
"""
    # 貼付版番号（BL-n）＝設定欄に貼られている起動文が最新かを、冒頭の1行で見分ける通し番号。
    # ユーザー提案（2026-09-09）：claude.ai とアプリの欄が同じ番号なら同期済みと一目で分かる。
    # 番号は**内容が変わったときだけ自動で1つ進む**（手動更新は書き忘れて必ず腐るため、
    # 番号を除いた本文のハッシュで変化を判定する）。full と mini は同じ番号を共有する——
    # 「すべての欄が同じ BL-n を名乗っていれば正しい」という単純な検算にするため。
    _rev_file = pathlib.Path('tools/boot_rev.json')
    _h = hashlib.sha256((boot + boot_mini).encode('utf-8')).hexdigest()[:16]
    try:
        _st = json.loads(_rev_file.read_text(encoding='utf-8'))
    except Exception:
        _st = {"hash": "", "rev": 0}
    if _st.get("hash") != _h:
        _st = {"hash": _h, "rev": int(_st.get("rev", 0)) + 1}
        _rev_file.write_text(json.dumps(_st, ensure_ascii=False) + "\n", encoding='utf-8')
    _bl = f'BL-{_st["rev"]}'
    boot = boot.replace('# 汎用マニュアル：常時適用（ブートローダー）',
                        f'# 汎用マニュアル：常時適用（ブートローダー full・貼付版 {_bl}）', 1)
    boot_mini = boot_mini.replace('# 汎用マニュアル：常時適用（短縮ブートローダー）',
                                  f'# 汎用マニュアル：常時適用（短縮ブートローダー mini・貼付版 {_bl}）', 1)
    (d / 'bootloader.md').write_text(boot, encoding='utf-8')
    (d / 'bootloader_mini.md').write_text(boot_mini, encoding='utf-8')
    print(f'  dist/bootloader_mini.md     {len(boot_mini.splitlines())} 行 / {len(boot_mini):,} 文字  ← Cowork の設定欄用（文字数上限対策）')
    print(f'  貼付版番号: {_bl}（full と mini で共通。内容が変わると自動で進む）')
    print(f'latest/ を更新した（{ver} / {date}）')
    print(f'  latest/L0_core_card.md      {(L / "L0_core_card.md").stat().st_size:,} バイト')
    print(f'  latest/manual_all_in_one.md {(L / "manual_all_in_one.md").stat().st_size:,} バイト')
    print(f'  latest/latest.json')
    print(f'  dist/bootloader.md          {len(boot.splitlines())} 行 / {len(boot):,} 文字  ← これを一度だけ貼る')
    return 0

if __name__ == '__main__':
    sys.exit(main())
