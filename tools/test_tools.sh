...
# 回帰（v60）：カード冒頭に旧版表記が残ると落ちる（2026-09-11 の実測——L2 参照が2版前のまま配布された）
cp "$CARD" "$TMP/bak2.md"
python3 - "$CARD" <<'PY'
import pathlib, sys
...
PY
python3 tools/build_dist.py > /dev/null 2>&1; chk "冒頭に旧版表記が残ると落ちる（異常系・v60）" 1 $?
cp "$TMP/bak2.md" "$CARD"
python3 tools/build_dist.py > /dev/null 2>&1; chk "復元後は再び合格する（v60）" 0 $?

echo "── make_handover.py ──"
python3 tools/make_handover.py --new "$TMP/h.md" > /dev/null 2>&1; chk "雛形を生成できる" 0 $?
python3 tools/make_handover.py --check dist/handover_template_v60.md > /dev/null 2>&1; chk "未記入テンプレートは不合格（異常系）" 1 $?
...
# ⑨-1・⑨-2 の必須スロット（v60）も「埋めれば合格」の対象である
...
# --- 回帰（v60）：理由を書き足すと指紋が外れる。--seal で封をし直せば --check が通ること ---
...
# --- 案件名の機械的な正規化（v60）：2026-09-02 の事案 ---
...
grep -q "受け取ったセッションが、最初にすること" "$TMP/hd.txt" && chk "引き継ぎの先頭に「最初にすること」が入る（v60）" 0 0 || chk "引き継ぎの先頭に「最初にすること」が入る（v60）" 0 1
...
# --- 枝分かれ（v60）：1つの作業が2つ以上のセッションへ分かれるとき ---
mkj2() { python3 -c "
import sys,pathlib,json
sid=sys.argv[2]
...
# 貼付版番号（v60）：full と mini の1行目に同じ BL-n が入り、再ビルドで番号が勝手に進まない
