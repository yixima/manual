...
# 回帰（v62）：カード冒頭に旧版表記が残ると落ちる（2026-09-11 の実測——L2 参照が2版前のまま配布された）
cp "$CARD" "$TMP/bak2.md"
python3 - "$CARD" <<'PY'
import pathlib, sys
...
PY
python3 tools/build_dist.py > /dev/null 2>&1; chk "冒頭に旧版表記が残ると落ちる（異常系・v62）" 1 $?
cp "$TMP/bak2.md" "$CARD"
python3 tools/build_dist.py > /dev/null 2>&1; chk "復元後は再び合格する（v62）" 0 $?

echo "── make_handover.py ──"
python3 tools/make_handover.py --new "$TMP/h.md" > /dev/null 2>&1; chk "雛形を生成できる" 0 $?
python3 tools/make_handover.py --check dist/handover_template_v62.md > /dev/null 2>&1; chk "未記入テンプレートは不合格（異常系）" 1 $?
...
# ⑨-1・⑨-2 の必須スロット（v62）も「埋めれば合格」の対象である
...
# --- 回帰（v62）：理由を書き足すと指紋が外れる。--seal で封をし直せば --check が通ること ---
...
# --- 案件名の機械的な正規化（v62）：2026-09-02 の事案 ---
...
grep -q "受け取ったセッションが、最初にすること" "$TMP/hd.txt" && chk "引き継ぎの先頭に「最初にすること」が入る（v62）" 0 0 || chk "引き継ぎの先頭に「最初にすること」が入る（v62）" 0 1
...
# --- 枝分かれ（v62）：1つの作業が2つ以上のセッションへ分かれるとき ---
mkj2() { python3 -c "
import sys,pathlib,json
sid=sys.argv[2]
...
# 貼付版番号（v62）：full と mini の1行目に同じ BL-n が入り、再ビルドで番号が勝手に進まない
