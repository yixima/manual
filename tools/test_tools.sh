...
python3 tools/audit_activation.py dist/L1_manual_v18.md --records dist/L2_records_v18.md > "$TMP/a.txt" 2>&1
chk "正常終了" 0 $?
...
cp dist/L0_core_card_v18.md "$TMP/bak.md"
printf '\n| わざと不一致にする行 | 検査が落ちることの確認 |\n' >> dist/L0_core_card_v18.md
python3 tools/build_dist.py > /dev/null 2>&1; chk "不一致があれば異常終了する（異常系）" 1 $?
cp "$TMP/bak.md" dist/L0_core_card_v18.md
...
python3 tools/make_handover.py --check dist/handover_template_v18.md > /dev/null 2>&1; chk "未記入テンプレートは不合格（異常系）" 1 $?
...
[ -f dist/L0_core_card_mini_v18.md ] && chk "短縮版が出力される" 0 0 || chk "短縮版が出力される" 0 1
grep -q "関門" dist/L0_core_card_mini_v18.md && chk "短縮版に関門が含まれる" 0 0 || chk "短縮版に関門が含まれる" 0 1
