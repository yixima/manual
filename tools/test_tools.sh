...
python3 tools/make_handover.py --check dist/handover_template_v43.md > /dev/null 2>&1; chk "未記入テンプレートは不合格（異常系）" 1 $?
...
# --- 回帰（v43）：理由を書き足すと指紋が外れる。--seal で封をし直せば --check が通ること ---
...
# --- 案件名の機械的な正規化（v43）：2026-09-02 の事案 ---
...
grep -q "受け取ったセッションが、最初にすること" "$TMP/hd.txt" && chk "引き継ぎの先頭に「最初にすること」が入る（v43）" 0 0 || chk "引き継ぎの先頭に「最初にすること」が入る（v43）" 0 1
...
# --- 枝分かれ（v43）：1つの作業が2つ以上のセッションへ分かれるとき ---
mkj2() { python3 -c "
import sys,pathlib,json
sid=sys.argv[2]
