...
# 7.5 「あるべきものが全部あるか」を数える（v39 で追加した §5 の規則を、自分にも適用する）
#     実例：他セッションが34頁中20頁でフッター欠落を納品まで見逃した。
#     原因は「はみ出しの有無」しか見ず、**あるべき要素の数を数えなかった**こと。
#     欠落は目に映らない。**数えなければ分からない。**
_m = re.search(r'## 5\. 機械的に強制される(\d+)つ', txt['L0'])
if _m:
    _want = int(_m.group(1))
    _sec = txt['L0'].split('## 5. 機械的に強制される')[1].split('## 5.5')[0]
    _got = len(re.findall(r'^\d+\. \*\*', _sec, re.M))
    check(_got == _want, f'L0 §5 の項目が表題どおり {_want} 件ある',
          f'表題は {_want} つだが本文は {_got} 件')
else:
    check(False, 'L0 §5 の表題から件数を読める', '「機械的に強制される<n>つ」の形になっていない')
