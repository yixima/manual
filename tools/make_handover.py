...
def auto(out, template, transcript=None, cwd=None, verbatim=True,
    ...
    L.append("> レート・単価・期限・環境・権限・パス・依存関係。**「言わなくても分かる」ものこそ書く。**\n"
             "> 姉妹案件・関連案件があるときは、その案件名と**最新の引き継ぎファイルのパス**をここに明記し、"
             "「当該案件の成果物を下敷きにするときは、そちらの引き継ぎの④と⑨を先に読むこと」と1行書く（v62・§10-5）。\n")
    ...
    L.append("")
    L.append("### 9-1. してはいけないこと（必須・v59）\n")
    L.append("> 禁止・制限・上限・順序の制約を、**根拠の条項番号つきで**列挙する。"
             "**参照した資料の条項番号を並べることでは満たさない。守るべき制約そのものを書く**"
             "（列挙は「読んだ記録」であって「制約の一覧」ではない。§10-5）。"
             "無いと確認したなら「なし（何と何を確認したか）」と書く。\n")
    L.append(f"- {TODO}\n")
    L.append("### 9-2. 起点となる日付（必須・v59）\n")
    L.append("> 開始日・締切・有効期限を、それぞれ**根拠つきで**列挙する。"
             "無いと確認したなら「なし（何を確認したか）」と書く。\n")
    L.append(f"- {TODO}\n")
    L.append("---\n")
