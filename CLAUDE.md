...
本リポジトリは「汎用マニュアル v36」とその発動・検証・改善の仕組みを管理する。
**本リポジトリでの作業にも、下記のコアカードをそのまま適用する**（自分で作ったルールを、自分が守らないことを禁じる）。

## リポジトリ固有の取り決め

- `dist/L1_manual_v36.md` と `dist/L2_records_v36.md` は **`tools/build_manual.py` が生成する**。直接編集しない。編集は build スクリプト側に書く（再現性のため）。
- `dist/L0_core_card_v36.md` は**手書きの原本**である。編集後は必ず `python3 tools/build_dist.py` で L1 との整合を検査する。
...
- 発行前に次の4つをすべて通す。1つでも落ちたら発行しない。
  ...
  python3 tools/audit_activation.py dist/L1_manual_v36.md --records dist/L2_records_v36.md   # 到達率100%・捕捉率100%
...
- **条項を追加する前に、必ず L1 §0-14（条項の定員制）を通す。** 既存条項で説明できるなら、条項ではなく発動経路を強化する。v36 で追加した条項は**0件**である。
